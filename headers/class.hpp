//ALL elements have nothing in common! The "Root" Entity class remains empty.
class Entity{};
...
class Fungi: public Grower{
	public:
		void multiply(Tile* ground){
			if(energy>3+num && num<4 && ground->temp>0 && rand()%100<(ground->water-400)/100){
				num++;
				energy-=3+num;
			}
		}
...
class Omnivore: public Animal{
	public:
		...
		void target(Tile** map){
			int new_x, new_y;
			//Check neighborhood for food/shroom to eat
			for(int i=-1;i<2;i++){
				for(int j=-1;j<2;j++){
					new_x = (j+x+W)%W;
					new_y = (i+y+H)%H;
